...
const StepCard = ({ icon, title, description, isLast = false }) => (
  <div className="flex items-start gap-6">
    <div className="flex flex-col items-center">
      <div className="flex items-center justify-center w-16 h-16 rounded-full bg-purple-100 dark:bg-purple-900/30 border-4 border-white dark:border-gray-900 shadow-md">
        {icon}
      </div>
      {!isLast && <div className="w-1 h-32 bg-purple-200 dark:bg-purple-800 mt-4 rounded"></div>}
    </div>
...
function HowItWorksPage() {
  const steps = [
    ...
    {
      icon: <BotMessageSquare className="h-8 w-8 text-purple-600 dark:text-purple-400" />,
    ...
    {
      icon: <BrainCircuit className="h-8 w-8 text-purple-600 dark:text-purple-400" />,
  ...
  return (
    <div className="bg-white dark:bg-gray-950 min-h-screen">
      <div className="container mx-auto px-4 py-20">
        ...
        >
          <h2 className="text-3xl font-bold text-gray-800 dark:text-white">Ready to Begin Your Ascent?</h2>
          <p className="mt-3 text-lg text-gray-600 dark:text-gray-400">Your first interview is just a click away.</p>
          <Button
            onClick={() => redirect("/Dashboard")}
            className="mt-8 text-xl font-semibold py-7 px-10 bg-purple-600 hover:bg-purple-700 text-white rounded-full shadow-lg shadow-purple-200 dark:shadow-purple-900/40 hover:shadow-xl transition-all transform hover:scale-105"
          >
